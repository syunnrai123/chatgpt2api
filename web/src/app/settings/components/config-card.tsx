                ...
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex justify-end">
                            ...
                            >
                                {isTestingProxy ? <LoaderCircle className="size-4 animate-spin"/> :
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            className="h-10 rounded-xl border-stone-200 bg-white"
                        />
                        <p className="text-xs text-stone-500">限制每个账号同时处理的图片请求数量，默认 3，最多 8。</p>
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm text-stone-700">文生图额度倍率</label>
                        <Input
                            type="number"
                            min="0.000001"
                            step="0.1"
                            value={String(config?.image_quota?.generation_multiplier ?? 1)}
                            onChange={(event) => setImageQuotaMultiplier("generation_multiplier", event.target.value)}
                            placeholder="1"
                            className="h-10 rounded-xl border-stone-200 bg-white"
                        />
                        <p className="text-xs text-stone-500">每生成 1 张文生图消耗的用户密钥额度。</p>
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm text-stone-700">图生图额度倍率</label>
                        <Input
                            type="number"
                            min="0.000001"
                            step="0.1"
                            value={String(config?.image_quota?.edit_multiplier ?? 1)}
                            onChange={(event) => setImageQuotaMultiplier("edit_multiplier", event.target.value)}
                            placeholder="1"
                            className="h-10 rounded-xl border-stone-200 bg-white"
                        />
                        <p className="text-xs text-stone-500">每生成 1 张图生图消耗的用户密钥额度。</p>
                    </div>
                    <label
                        className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
                        <Checkbox
                    ...
                    <div className="space-y-3 rounded-xl border border-stone-200 bg-white px-4 py-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {logLevelOptions.map((level) => (
                                <label key={level}
                                       className="flex items-center gap-2 text-sm capitalize text-stone-700">
                                    <Checkbox
                    ...
                    <div className="space-y-2 md:col-span-2">
                        ...
                        />
                        <p className="text-xs text-stone-500">每次请求都会作为 system
                            消息注入，可用于审核用户提示词、避免违规内容、统一约束模型行为或固定角色设定。</p>
                    </div>
                    <div className="space-y-2 md:col-span-2">
                    ...
                    <div className="space-y-4 rounded-xl border border-stone-200 bg-white px-4 py-3 md:col-span-2">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                >
                                    {isTestingImageStorage ? <LoaderCircle className="size-4 animate-spin"/> :
                                ...
                                >
                                    {isSyncingImageStorage ? <LoaderCircle className="size-4 animate-spin"/> :
                        ...
                        <div className="grid gap-4 md:grid-cols-3">
                            ...
                            <div className="space-y-2 md:col-span-3">
                                ...
                                <Input
                                    ...
                                    disabled={!config?.image_storage?.enabled}
                                />
                                <p className="text-xs text-stone-500">留空时返回本应用 /images/...
                                    代理地址；填入后直接返回公开图片地址。</p>
                            </div>
                        </div>
                    </div>
                    <div className="space-y-4 rounded-xl border border-stone-200 bg-white px-4 py-3 md:col-span-2">
                        ...
                        <div className="grid gap-4 md:grid-cols-3">
                            <div className="space-y-2">
                                ...
                                <Input value={String(config?.ai_review?.base_url || "")}
                            ...
                            <div className="space-y-2">
                                ...
                                <Input value={String(config?.ai_review?.api_key || "")}
                                       onChange={(event) => setAIReviewField("api_key", event.target.value)}
                                       placeholder="sk-..." className="h-10 rounded-xl border-stone-200 bg-white"/>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm text-stone-700">Model</label>
                                <Input value={String(config?.ai_review?.model || "")}
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea value={String(config?.ai_review?.prompt || "")}
