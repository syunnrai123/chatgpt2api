...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
    ...
    return {
        ...
        image_quota: {
            generation_multiplier: Number(config.image_quota?.generation_multiplier ?? 1),
            edit_multiplier: Number(config.image_quota?.edit_multiplier ?? 1),
        },
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
    ...
    saveConfig: async () => {
        ...
        try {
            const data = await updateSettingsConfig({
                ...
                image_quota: {
                    generation_multiplier: Math.max(0.000001, Number(config.image_quota?.generation_multiplier) || 1),
                    edit_multiplier: Math.max(0.000001, Number(config.image_quota?.edit_multiplier) || 1),
                },
                auto_remove_invalid_accounts: Boolean(config.auto_remove_invalid_accounts),
                auto_remove_rate_limited_accounts: Boolean(config.auto_remove_rate_limited_accounts),
    ...
    setImageQuotaMultiplier: (key, value) => {
        set((state) => state.config ? {
            config: {
                ...state.config,
                image_quota: {
                    ...(state.config.image_quota || {}),
                    [key]: value,
                },
            },
        } : {});
