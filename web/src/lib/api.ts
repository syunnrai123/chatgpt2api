...
export type SettingsConfig = {
    ...
    image_quota?: {
        generation_multiplier?: number | string;
        edit_multiplier?: number | string;
    };
...
export async function fetchCurrentIdentity() {
    return httpRequest<CurrentIdentity>("/api/auth/me");
}
...
export async function generateImage(prompt: string, model?: ImageModel, size?: string, resolution?: ImageResolution) {
...
export async function editImage(files: File | File[], prompt: string, model?: ImageModel, size?: string, resolution?: ImageResolution) {
...
export async function rechargeUserKeyQuota(keyId: string, amount: number) {
    return httpRequest<{ item: UserKey; items: UserKey[] }>(`/api/auth/users/${keyId}/quota`, {
        method: "POST",
        body: {amount},
    });
}
